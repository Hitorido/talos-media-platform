...
const enabled={mangadex:true,narou:true,'anilist-anime':true};
...
const novel=feeds.find(s=>s.id==='trendingNovels').items[0];const response=await fetch(base+'/api/content/novel/narou/'+encodeURIComponent(novel.sourceId),{signal:AbortSignal.timeout(20000)});assert.equal(response.status,200,'Discovered novel details route');
const invalid=await fetch(base+'/api/content/discovery/narou?feed=arbitrary');assert.equal(invalid.status,400);
