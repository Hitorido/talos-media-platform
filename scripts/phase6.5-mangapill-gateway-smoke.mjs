...
for(const url of [d.coverUrl,pages[0].imageUrl]){const r=await fetch(url);assert.equal(r.status,200);assert.equal(r.headers.get('cross-origin-resource-policy'),'cross-origin');assert.match(r.headers.get('content-type'),/^image\//);await r.body?.cancel();}
