...
for(const query of ['mediaId=../secret','mediaId=koi_wa_amaagari_no_you_ni&url=https://example.com','mediaId=koi_wa_amaagari_no_you_ni&chapterId=c001&page=301','mediaId=koi_wa_amaagari_no_you_ni&page=2','mediaId=https://localhost']){const r=await fetch(base+'/api/content/mangatown/image?'+query);assert.equal(r.status,400);}
