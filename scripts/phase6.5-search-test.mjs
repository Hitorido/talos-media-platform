...
providers[0].getChapters=async()=>[];
await service.getMediaChapters('comic__Solo Leveling');
assert.ok(!(await service.unifiedSearch('error','manga')).results.some(x=>x.title==='Solo Leveling'),'confirmed empty list hidden even from cached search');
providers[0].getChapters=async()=>[{id:'1'}];await service.getMediaChapters('comic__Solo Leveling');
assert.ok((await service.unifiedSearch('error','manga')).results.some(x=>x.title==='Solo Leveling'),'successful retry restores title');
console.log('PASS confirmed empty comic filtering and successful recovery without search enrichment');
