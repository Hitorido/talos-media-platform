...
/** Five bounded feed operations, rather than querying every installed source. */
export function getDiscovery(enabled:Record<string,boolean>, refresh=false):Promise<DiscoverySection[]>{
 const key=getApiBaseUrl()+'|'+['mangadex','narou','anilist-anime'].filter(id=>enabled[id]).join(',');
 ...
 const operation=(async()=>{
  ...
  const set=(id:string,items:DiscoveryItem[])=>{output.find(section=>section.id===id)!.items=unique(items);};
  ...
  if(enabled.narou)for(const [section,feed] of [['trendingNovels','popular'],['recentNovels','updated']])jobs.push({providerId:'narou',sections:[section],run:async()=>{
