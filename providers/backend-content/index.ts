...
export function backendComicProvider(id: string, name: string): MediaProvider {
  ...
  return {
    ...
    async search(query, context) {
      ...
      return data.results.map(item=>({id:encodeMediaRouteId(id,item.sourceId),providerId:id,sourceId:item.sourceId,title:item.title,coverUrl:item.coverUrl??'',type:'manga' as const,comicFormat:(['manhwa','manhua'].includes(item.mediaType)?item.mediaType:'manga') as 'manga'|'manhwa'|'manhua',subtitle:name,tags:[name]}));
    },
